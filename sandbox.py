...
def update_day(index):
    ...
    exerciseRadioBtn.on_clicked(test)
    plt.draw()

def test(index):
    rex = plt.axes([0.9, 0.7, 0.24, 0.24])
    xxx = RadioButtons(rex,("Okay","No"), active=0)
    xxx.on_clicked(contains_semicolon)
    plt.draw()
def next(event):
